...
""".format(
    ...
    def test_{test_id}(self, page):
        ...
        try:
            logger.info(f"开始测试: {test_id} - {element_type}")
            ...
            if "{element_type}" == "button":
                ...
                logger.info(f"测试通过: {test_id} - 下拉菜单选择成功，选中值: {selected_value}")
                ...
                logger.info(f"测试通过: {test_id} - 表单分析成功")
                
        except Exception as e:
            logger.error(f"测试失败: {test_id} - {str(e)}")
            # 尝试截图
            try:
